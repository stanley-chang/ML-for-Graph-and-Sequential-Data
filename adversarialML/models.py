...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        counts_selection = self._sample_noise(inputs, n0, batch_size)
        cAHat = counts_selection.argmax().item()
        counts_estimation = self._sample_noise(inputs, num_samples, batch_size)
        nA = counts_estimation[cAHat].item()
        p_A_lower_bound = self._lower_confidence_bound(nA, num_samples, alpha)
        ##########################################################
